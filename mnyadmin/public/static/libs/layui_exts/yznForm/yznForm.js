...
layui.define(['layer', 'form', 'yzn', 'table', 'notice', 'element', 'dragsort'], function(exports) {
    ...
    var yznForm = {
        ...
        events: {
            ...
            //-------------------------S 添加节点结束-----------------------------------------
            courselist: function(layform) {
                console.log(333);
                // 绑定courselist组件
                if ($(".courselist",layform).size() > 0) {
                    layui.define('laytpl', function(exports) {
                        var laytpl = layui.laytpl;
                        //刷新隐藏textarea的值
                        var refresh = function(name, obj) {
                            var data = {};
                            var textarea = $("textarea[name='" + name + "']",layform);
                            var container = $(".courselist[data-name='" + name + "']");
                            var template = container.data("template");
                            $.each($("input,select,textarea", container).serializeArray(), function(i, j) {
                                var reg = /\[(\w+)\]\[(\w+)\]$/g;
                                var match = reg.exec(j.name);
                            ...
                            // return false;
                            // textarea.val('33333');
                            // console.dir($(textarea[0]).val());
                            textarea.val(JSON.stringify(result));
                            // textarea.val();
                            // console.dir($(textarea[0]).val());
                        };
                        //监听文本框改变事件
                        $(document).on('change keyup changed', ".courselist input,.courselist textarea,.courselist select", function() {
                            refresh($(this).closest(".courselist").data("name"));
                        });

                        //追加控制
                        $(".courselist",layform).on("click", ".btn-append,.append", function(e, row) {
                            var container = $(this).closest(".courselist");
                            var tagName = container.data("tag") || (container.is("table") ? "tr" : "dd");
                            var index = container.data("index");
                            var name = container.data("name");
                            var id = container.data("id");
                            ...
                            laytpl($("#" + id + "Tpl").html()).render(vars, function(html) {
                                $(html).attr("courselist-item", true).insertBefore($(tagName + ":last", container));
                            });
                            form.render();
                            // $(this).trigger("fa.event.courselist", $(this).closest(tagName).prev());
                        });
                        //移除控制
                        $(".courselist",layform).on("click", ".btn-remove", function() {
                            var container = $(this).closest(".fieldlist");
                            var tagName = container.data("tag") || (container.is("table") ? "tr" : "dd");
                            $(this).closest(tagName).remove();
                            refresh(container.data("name"));
                        ...
                        //渲染数据&拖拽排序
                        $(".courselist",layform).each(function() {
                            var container = this;
                            var tagName = $(this).data("tag") || ($(this).is("table") ? "tr" : "dd");
                            $(this).dragsort({
                                itemSelector: tagName,
                            ...
                            textarea.on("fa.event.courselist", function() {
                                $("[courselist-item]", container).remove();
                                var json = {};
                                try {
                                    json = JSON.parse($(this).val());
                                } catch (e) {}
                            ...
                            // return false;
                            textarea.trigger("fa.event.courselist");
                            // return false;
                        });
                    })
                }
